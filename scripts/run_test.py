...
def main():
    ...
    try:
        while (per_env_episode_count < per_env_target_episodes).any():
            with torch.inference_mode():
                ...
                # 2. 物理仿真（瞬移在这里生效；terminated/truncated 的环境会自动 reset，
                #    reset 内部触发 _compute_total_metrics，写入 eval_metrics）
                obs, rew, terminated, truncated, info = env.step(zero_actions)
                step_count += 1

                # 3. 检测 Tester 认为已完成测试的环境
                tester_completed_envs = tester.get_completed_envs()

                # 4. 合并自动重置和测试完成的环境（去重）
                reset_mask = terminated | truncated
                env_reset_ids = torch.where(reset_mask)[0]
                all_reset_ids = torch.unique(
                    torch.cat([env_reset_ids, tester_completed_envs])
                )

                # 5. 只处理未达到目标轮数的环境
                if len(all_reset_ids) == 0:
                    if step_count % 200 == 0:
                        total_completed = per_env_episode_count.sum().item()
                        print(f"[Main] Step {step_count} | "
                              f"各环境episode数: {per_env_episode_count.tolist()} | "
                              f"总计: {total_completed}")
                    continue
                ...
                if len(valid_reset_ids) > 0:
                    print(f"\n[Main] Step {step_count}: "
                          f"{len(valid_reset_ids)} 个环境即将重置")

                    # A. 对 tester 完成但未自动重置的环境，强制触发 env reset
                    #    （_reset_idx 内部调用 command_term.reset → _compute_total_metrics）
                    force_reset_ids = []
                    for eid in tester_completed_envs:
                        eid_val = eid.item() if isinstance(eid, torch.Tensor) else eid
                        if eid_val in valid_reset_ids.tolist() and eid_val not in env_reset_ids.tolist():
                            force_reset_ids.append(eid_val)

                    if force_reset_ids:
                        force_tensor = torch.tensor(
                            force_reset_ids, dtype=torch.long, device=env.device,
                        )
                        env.unwrapped._reset_idx(force_tensor)

                    # B. 快照指标（此时所有待重置环境的 eval_metrics 已由 reset 填充）
                    tester.snapshot_metrics(valid_reset_ids)

                    # C. 重置 Tester 状态，准备下一个 episode
                    ...
                    total_completed = per_env_episode_count.sum().item()
                    print(f"[Main] 进度: {per_env_episode_count.tolist()} "
                          f"(总计 {total_completed}/"
                          f"{per_env_target_episodes * env.num_envs})\n")
    ...
    print(f"各环境episode数: {per_env_episode_count.tolist()} "
          f"(目标: 每个环境 {per_env_target_episodes} 轮)")
