...
def update_tray_positions(env, env_ids, tray_or_not):
    ...
    for box_idx, box_asset in enumerate(source_box_assets):
        ...
        if tray_or_not[box_idx]:
            ...
            # 托盘 1
            pos_val1 = [0.0, -0.14, 0.051] 
            relative_pos1 = torch.tensor(pos_val1, device=env.device).unsqueeze(0).repeat(len(env_ids), 1)
            # 直接创建一个形状为 (1, 4) 的张量，然后复制 N 份
            # [1, 0, 0, 0] 是标准的无旋转四元数 (w, x, y, z)
            relative_quat = torch.tensor([[1.0, 0.0, 0.0, 0.0]], device=env.device).repeat(len(env_ids), 1)

            tray_object_obj1 = env.scene[f"tray_{box_idx*2}"]
            set_asset_relative_position(
                env=env, # 修正 3: 使用传入的 env
                env_ids=env_ids,
                target_asset=tray_object_obj1,
                reference_asset=box_asset_obj,
                relative_pos=relative_pos1,
                relative_quat=relative_quat
            )
            
            # 托盘 2
            tray_object_obj2 = env.scene[f"tray_{box_idx*2+1}"]
            pos_val2 = [0.0, 0.14, 0.051] 
            relative_pos2 = torch.tensor(pos_val2, device=env.device).unsqueeze(0).repeat(len(env_ids), 1)
            set_asset_relative_position(
                env=env,
                env_ids=env_ids,
                target_asset=tray_object_obj2,
                reference_asset=box_asset_obj,
                relative_pos=relative_pos2,
                relative_quat=relative_quat
            )
