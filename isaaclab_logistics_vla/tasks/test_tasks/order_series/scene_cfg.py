...
class OrderSceneCfg(InteractiveSceneCfg):
    ...
    s_box_1 = RigidObjectCfg(
        ...
        spawn=UsdFileCfg(usd_path=BOX_USD_PATH, mass_props=sim_utils.MassPropertiesCfg(mass=5.0), scale=(1, 1, 1), rigid_props=schemas.RigidBodyPropertiesCfg(sleep_threshold=0.05)),
        init_state=RigidObjectCfg.InitialStateCfg(pos=(1.57989, 1.33474, 0.755)),
    )
    s_box_2 = RigidObjectCfg(
        prim_path="{ENV_REGEX_NS}/s_box_2",
        spawn=UsdFileCfg(usd_path=BOX_USD_PATH, mass_props=sim_utils.MassPropertiesCfg(mass=5.0), scale=(1, 1, 1), rigid_props=schemas.RigidBodyPropertiesCfg(sleep_threshold=0.05)),
        init_state=RigidObjectCfg.InitialStateCfg(pos=(1.025, 1.33614, 0.755)),
    )
    s_box_3 = RigidObjectCfg(
        prim_path="{ENV_REGEX_NS}/s_box_3",
        spawn=UsdFileCfg(usd_path=BOX_USD_PATH, mass_props=sim_utils.MassPropertiesCfg(mass=5.0), scale=(1, 1, 1), rigid_props=schemas.RigidBodyPropertiesCfg(sleep_threshold=0.05)),
        init_state=RigidObjectCfg.InitialStateCfg(pos=(0.51025, 1.33614, 0.755)),
    )

    t_box_1 = RigidObjectCfg(
        prim_path="{ENV_REGEX_NS}/t_box_1",
        spawn=UsdFileCfg(usd_path=BOX_USD_PATH, mass_props=sim_utils.MassPropertiesCfg(mass=5.0), scale=(1, 1, 1), rigid_props=schemas.RigidBodyPropertiesCfg(sleep_threshold=0.05)),
        init_state=RigidObjectCfg.InitialStateCfg(pos=(1.57989, 3.4429, 0.66)),
    )
    t_box_2 = RigidObjectCfg(
        prim_path="{ENV_REGEX_NS}/t_box_2",
        spawn=UsdFileCfg(usd_path=BOX_USD_PATH, mass_props=sim_utils.MassPropertiesCfg(mass=5.0), scale=(1, 1, 1), rigid_props=schemas.RigidBodyPropertiesCfg(sleep_threshold=0.05)),
        init_state=RigidObjectCfg.InitialStateCfg(pos=(1.025, 3.4429, 0.66)),
    )
    t_box_3 = RigidObjectCfg(
        prim_path="{ENV_REGEX_NS}/t_box_3",
        spawn=UsdFileCfg(usd_path=BOX_USD_PATH, mass_props=sim_utils.MassPropertiesCfg(mass=5.0), scale=(1, 1, 1), rigid_props=schemas.RigidBodyPropertiesCfg(sleep_threshold=0.05)),
        init_state=RigidObjectCfg.InitialStateCfg(pos=(0.510, 3.4429, 0.66)),
    )
